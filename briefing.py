...
- KakaoTalk MemoChat (PlayMCP): 핵심 요약 200자 발송
...
PLAYMCP_TOKEN       = os.environ["PLAYMCP_TOKEN"]        # PlayMCP API 토큰
...
# ── 2. Claude API로 뉴스 수집 + 브리핑 생성 ──────────────
def fetch_briefing(prices: dict) -> dict:
    """
    Claude API (web_search 도구 포함)를 호출해
    뉴스 수집 + 카톡 요약(≤200자) + 이메일 풀 브리핑 HTML을 한 번에 반환.
    """
    price_summary = "\n".join(
...
{{
  "kakao_message": "200자 이내 핵심 요약. 종가 + 주요뉴스 핵심 포인트. 이모지 활용.",
  "email_subject": "이메일 제목 (예: [포트폴리오 브리핑] 2026.06.03)",
  "email_html": "뉴스 요약 + 주가 포함한 풀 브리핑 HTML. 깔끔한 스타일 인라인 적용."
}}"""
...
def send_kakao(message: str):
    ...
    resp = requests.post(
        "https://playmcp.kakao.com/mcp/v1/kakao/memo",
        headers={
            "Authorization": f"Bearer {PLAYMCP_TOKEN}",
            "Content-Type": "application/json",
        },
...
def main():
    ...
    print("📲 카카오톡 전송 중...")
    send_kakao(briefing["kakao_message"])
